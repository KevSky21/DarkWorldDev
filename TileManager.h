...
private:
    ...
    float m_fTileSize = 32.0f; ///< Tile width and height (adjust later).
...
public:
    CTileManager(LSpriteRenderer* renderer);
    ~CTileManager();

    void LoadMap(const char* filename); ///< Load a map from text file
    void Draw();                        ///< Draw all tiles

	bool CheckCollision(const Vector2& pos, float radius); ///< Check for collision with solid tiles
};